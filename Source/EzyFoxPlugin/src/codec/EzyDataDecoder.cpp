...
void EzyDataDecoder::addData(const char* data, int size) {
	...
	uint32_t dataSize = (int)mDataBuffer.size();
	while (true) {
...
int EzyDataDecoder::processData(const char* buffer, uint32_t& dataSize) {
	if (dataSize <= 0) {
		return 0;
	}
	...
	if (0x00 <= type && type <= 0x7f) {
		...
		//fixstr
		uint32_t  n = (type & 0x1f);// (type & ~0xe0);
		if (dataSize >= n + 1) {
		...
		//str 16
		if (dataSize >= 3) {
			uint32_t n = readUInt16(buffer + 1);
			if (dataSize >= n + 3) {
